...
void SigVisualizer::plot_signal(const Wav::Signal &sig, const struct Screen &screen)
{
    ...
    for (size_t i = 0; i < drawable_width; i++)
    {
        ...
        float x_pos = static_cast<float>(i + screen.W_PADDING);

        Vector2 start_pos = Vector2{.x = x_pos, .y = (screen.HEIGHT / 2) - y_pos};
        Vector2 end_pos = Vector2{.x = x_pos, .y = (screen.HEIGHT / 2) + y_pos};
