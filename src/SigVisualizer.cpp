...
namespace SigVisualizer
{
    void plot_axis_t_sig(const struct Screen& screen, const SigProccesing::t_signal& sig)
    {
        ...
        for (int i = 0; i <= sig_duration; i++) // Iterate based on the signal duration
        {
            ...
            DrawTextEx(GetFontDefault(), TextFormat("%i", i), {
                           .x = static_cast<float>(x_pos) - 3, .y = static_cast<float>(screen.HEIGHT - 35)
                       }, 15, 1, WHITE);
        }
    ...
    void plot_signal(const SigProccesing::t_signal& sig, const struct Screen& screen, plot_types plot_type)
    {
        ...
        double y_scale = (screen.HEIGHT - 2 * screen.H_PADDING) / (2 * largest_val_in_sig);
        // std::cout << y_scale << "\n";
        ...
        if (plot_type == WAVEFORM)
        {
            ...
            {
                int idx = i * step_size;
                double y_pos = std::abs(SigProccesing::mooving_avg(sig.samples, step_size, idx, y_scale));
